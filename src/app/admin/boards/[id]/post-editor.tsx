...
export default function PostEditor({ post, replies }: { post: PostData; replies: Reply[] }) {
  ...
  const inputCls = "w-full rounded-lg border border-slate-300 bg-slate-100 px-3 py-2 text-sm text-slate-800 outline-none focus:border-[#3182f6]/50 focus:ring-1 focus:ring-[#3182f6]/30 disabled:opacity-50 disabled:cursor-not-allowed";
        ...
        {!editing ? (
          <>
            <button
              ...
              className="rounded-lg bg-[#3182f6] px-4 py-2 text-sm font-medium text-white hover:bg-[#3182f6]"
            >
              수정
            </button>
            <button
          ...
          <>
            <button
              onClick={handleSave}
              disabled={isPending}
              className="rounded-lg bg-[#3182f6] px-4 py-2 text-sm font-medium text-white hover:bg-[#3182f6] disabled:opacity-50"
            >
              {isPending ? "저장중..." : "저장"}
            </button>
            <button
          ...
          <div className="flex items-end gap-4">
            <label className="flex items-center gap-2 text-sm text-slate-700">
              <input
                ...
                disabled={!editing}
                className="rounded border-slate-300 bg-slate-100 text-[#3182f6]"
              />
              공지사항
            </label>
            <label className="flex items-center gap-2 text-sm text-slate-700">
