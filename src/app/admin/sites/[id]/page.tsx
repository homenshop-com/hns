  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        <Link href="/admin/sites" className="text-cyan-400 hover:text-cyan-300 text-sm">&larr; Back to list</Link>
        <h1 className="text-xl font-bold text-slate-100">Account Detail: {site.shopId}</h1>
        <span className={`inline-block rounded-md px-3 py-1 text-xs font-medium ${
          isExpired ? "bg-red-500/10 text-red-400" : site.accountType === '1' ? "bg-emerald-500/10 text-emerald-400" : "bg-cyan-500/10 text-cyan-400"
        }`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Account Info */}
        <div className="bg-[#1e293b]/80 rounded-xl border border-slate-700/30 p-6">
          <h2 className="text-base font-semibold text-slate-200 mb-4">Account Info</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex gap-2 items-center">
                  <input name="shopId" defaultValue={site.shopId} className="flex-1 border border-slate-600/40 rounded-lg bg-slate-800/50 px-3 py-2 text-sm text-slate-200" />
                  <a href={`https://home.homenshop.com/${site.shopId}`} target="_blank" rel="noopener noreferrer" className="shrink-0 rounded-lg border border-slate-600/40 px-3 py-2 text-xs text-cyan-400 hover:bg-slate-800/50 hover:text-cyan-300 transition-colors">Open ↗</a>
                </div>
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">Site Name</label>
                <input name="name" defaultValue={site.name} className="w-full border border-slate-600/40 rounded-lg bg-slate-800/50 px-3 py-2 text-sm text-slate-200" />
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">Owner</label>
                <Link href={`/admin/members/${site.user.id}`} className="text-cyan-400 hover:text-cyan-300">
                  {site.user.email}
                </Link>
              </div>
              <div>
              ...
              <div>
                ...
                <input type="date" name="expiresAt" defaultValue={site.expiresAt ? site.expiresAt.toISOString().split("T")[0] : ""} className="w-full border border-slate-600/40 rounded-lg bg-slate-800/50 px-3 py-2 text-sm text-slate-200" />
              </div>
        ...
        <div className="space-y-6">
          {/* Extend */}
          <div className="bg-[#1e293b]/80 rounded-xl border border-slate-700/30 p-6">
            <h2 className="text-base font-semibold text-slate-200 mb-4">Extend Account</h2>
            <form action={extendSite} className="flex items-end gap-4">
              <input type="hidden" name="id" value={site.id} />
              <div>
                <label className="block text-xs text-slate-500 mb-1">Period</label>
                <select name="months" className="border border-slate-600/40 rounded-lg bg-slate-800/50 px-3 py-2 text-sm text-slate-200">
          ...
          {/* Domains */}
          <div className="bg-[#1e293b]/80 rounded-xl border border-slate-700/30 p-6">
            <h2 className="text-base font-semibold text-slate-200 mb-4">Domains ({site.domains.length})</h2>
            {site.domains.length > 0 ? (
              <ul className="space-y-2">
                {site.domains.map((d) => (
                  <li key={d.id} className="text-sm"><a href={`https://${d.domain}`} target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:text-cyan-300">{d.domain} ↗</a></li>
                ))}
              </ul>
            ) : (
              <p className="text-sm text-slate-400">No custom domains</p>
            )}
          </div>

          {/* Pages */}
          <div className="bg-[#1e293b]/80 rounded-xl border border-slate-700/30 p-6">
            <h2 className="text-base font-semibold text-slate-200 mb-4">Pages ({site.pages.length})</h2>
          ...
          {/* Delete */}
          <div className="bg-[#1e293b]/80 rounded-xl border border-red-500/30 p-6">
            <h2 className="text-base font-semibold text-slate-200 mb-2 text-red-400">Danger Zone</h2>
