# homenshop.com — Design System Reference

> Primary design reference for homenshop.com. Adapted from Toss — Playful Fintech (Korean-first). All new UI work should follow these tokens and patterns. Existing v2 CSS files (`dashboard-v2.css`, `settings-v2.css`, `manage-v2.css`, `menus-v2.css`, `profile-v2.css`, `credits-v2.css`, `domains-v2.css`, `support-v2.css`) and `landing.css` should be migrated to these tokens.
>
> Source: rohitg00/awesome-claude-design — `design-md/playful/toss.md` (2026-05-12 imported).
...
1. **Desktop-first dashboards**: homenshop.com administrative pages (`/dashboard/*`, `/admin/*`) are desktop-first (1200–1440px shells), not the canonical Toss 480px mobile lane. Marketing and end-user pages (`/`, `/pricing`, `/login`, published sites) stay mobile-first.

2. **No "Toss Product Sans"**: licensing constraint — substitute the canonical heading stack with `Pretendard Variable, Pretendard, -apple-system, BlinkMacSystemFont, "Apple SD Gothic Neo", "Noto Sans KR", system-ui, sans-serif`. Weights and metrics remain the same.

3. **Numbers**: keep `font-variant-numeric: tabular-nums` everywhere — credit balances, page counts, expiry days, dates, KPI deltas.

4. **Status colors** (`--success`, `--warning`, `--danger`) are state-only — never for decoration, never for ribbons, never for category labels. Reuse `--accent` + `--accent-soft` for any decorative emphasis.

5. **Site monogram tiles**: when the dashboard renders user sites, the tile background MUST be `--accent-soft` (#e8f3ff), text in `--accent-deep`. Do not rotate through multiple hues — Toss's "single confident blue" rule wins.

6. **Gradient ban scope**: applies to dashboard/admin chrome and marketing hero. Published user sites (which are designer-controlled) may still use gradients — that's user content.

7. **Migration order** (existing v2 CSS files to update):
   1. `dashboard-v2.css` (Phase 1, in progress)
   2. `manage-v2.css` + `menus-v2.css` (data + menu management)
   3. `settings-v2.css` (site settings)
   4. `profile-v2.css` (admin info)
   5. `credits-v2.css` (AI credits)
   6. `domains-v2.css` (domain management)
   7. `support-v2.css` (help)
   8. `landing.css` (templates gallery + auth + landing)

Each file's `:root` (or scope) variables map onto the Toss tokens above. Component patterns (buttons, cards, inputs, tables) are rewritten to match the Toss component rules in section 4.
